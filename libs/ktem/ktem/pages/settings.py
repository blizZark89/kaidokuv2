...
class SettingsPage(BasePage):
    ...
    **IMPORTANT**: the name and id of the UI setting components should match the
    ...
    def change_password(self, user_id, password, password_confirm):
        ...
        with Session(engine) as session:
            ...
            if result:
                ...
                session.add(user)
                session.commit()
                gr.Info("Passwort wurde geändert")
            else:
    ...
    def reasoning_tab(self):
        with gr.Tab("Denk-Einstellungen", visible=self._render_reasoning_tab):
            with gr.Group():
            ...
            gr.Markdown("### Denkmodus-spezifische Einstellungen")
    ...
    def save_setting(self, user_id: int, *args):
        ...
        if user_id is None:
            gr.Warning("Bitte vor dem Speichern anmelden")
            return setting
