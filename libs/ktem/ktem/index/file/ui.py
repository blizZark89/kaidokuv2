...
class DirectoryUpload(BasePage):
    ...
    def on_building_ui(self):
        with gr.Accordion(label="Verzeichnis-Upload", open=False):
            gr.Markdown(f"Unterstützte Dateitypen: {self._supported_file_types_str}")
            self.path = gr.Textbox(
                placeholder="Verzeichnispfad...", lines=1, max_lines=1, container=False
            )
            with gr.Accordion("Erweiterte Index-Optionen", open=False):
                with gr.Row():
    ...
    def render_file_list(self):
        self.filter = gr.Textbox(
            ...
            info=(
                ...
                "(2) Eine leere Suche zeigt alle Dateien."
            ),
        )
            ...
            with gr.Column(scale=1):
                with gr.Column() as self.upload:
                    ...
                    with gr.Tab("Web-Links verwenden"):
                        ...
                        gr.Markdown("(durch Zeilenumbrüche getrennt)")
                ...
                with gr.Column(visible=False) as self.upload_progress_panel:
                    ...
                    with gr.Row():
                        ...
                        self.upload_info = gr.Textbox(
                            lines=1, max_lines=20, label="Upload-Informationen"
                        )
                    self.btn_close_upload_progress_panel = gr.Button(
                        "Upload-Infos leeren und schließen",
                        variant="secondary",
